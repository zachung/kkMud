 ...
 * (SimpleTelnetOptionHandler) is used. Initial configuration requested by TelnetClient will be:
 * WILL ECHO, WILL SUPPRESS-GA, DO SUPPRESS-GA. VT100 terminal type will be subnegotiated.
 * <p>
 * Also, use of the sendAYT(), getLocalOptionState(), getRemoteOptionState() is demonstrated. When
 * connected, type AYT to send an AYT command to the server and see the result. Type OPT to see a
 * report of the state of the first 25 options.
 ***/
public class application implements Runnable, TelnetNotificationHandler {
  ...
  public void main(String[] args) throws Exception {
    ...
    // termType, initLocal, initRemote, acceptLocal, acceptremote
    ...
    while (true) {
      ...
      try {
        ...
        Thread reader = new Thread(new application());
        tc.registerNotifHandler(new application());
        ...
        do {
          try {
            ...
            if (ret_read > 0) {
              final String line = new String(buff, 0, ret_read); // deliberate use of default
                                                                 // charset
